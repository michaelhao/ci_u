...
$articles=$this->db->order_by('sort','desc')->get_where('article', array(
	'Recover' => 0, 
		...
		<table class="table">
			...
			<tbody>
				...
				?>
					<tr>
						<td>
							<input id="list_sort_<?=$article["id"]?>" type="text" class="form-control list_sort" value="<?=$article["sort"]?>">
							<span class="btn btn-icon btn-info btn-xs" onclick="sortUrl('article/sort','<?=$article["id"]?>')">
								送出
							</span>
						</td>
						<td>
